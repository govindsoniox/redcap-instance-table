...
<?php
     ...
     */
      ...
      class InstanceTable extends AbstractExternalModule
      {
              ...
              const ACTION_TAG_HIDEEMPTY = '@INSTANCETABLE[-_]HIDEEMPTY'; // Hide the entire table when no data is available
                      ...
                      foreach ($instrumentFields as $instrumentField => $fieldDetails) {
                              ...
                              if ($fieldDetails['field_type']==='descriptive' &&
                                  preg_match("/".self::ACTION_TAG."\s*=\s*'?((\w+_arm_\d+[a-z]?:)?\w+)'?\s?/", $fieldDetails['field_annotation'], $matches)) {
                                      ...
                                      // pick up option for hiding empty tables
                                      $matches = array();
                                      if (preg_match("/".self::ACTION_TAG_HIDEEMPTY."\s?/", $fieldDetails['field_annotation'], $matches)) {
                                          $repeatingFormDetails['hide_empty_table'] = true;
                                      } else {
                                          $repeatingFormDetails['hide_empty_table'] = false;
                                      }
                  ...
                  if (isSurvey) {
                      thisTbl = $('#'+taggedField.html_table_id)
                          .DataTable( {
                              ...
                              "language": {
                                  "emptyTable": (taggedField.hide_empty_table ? "" : "No data available in table")
                              },
                              "drawCallback": function(settings) {
                                  if (taggedField.hide_empty_table && this.api().page.info().recordsDisplay === 0) {
                                      $('#'+taggedField.html_table_id).closest('div').hide();
                                  } else {
                                      $('#'+taggedField.html_table_id).closest('div').show();
                                  }
                              }
                          } );
